...
- **T.3** A same-sleeve (fused) card (Z.7) has **no independent zone existence** — it is a child of its host, not a card occupying any zone of its own. Ownership (T.2) therefore never brings it home: it moves and rests with its host wherever the host goes, including into an opponent's zones when cast on an opponent's creature (P.27, P.29), for the remainder of the game. It is not counted, targeted, or moved as an independent object (C.16, C.4, V.7b). T.2 is not overridden so much as rendered moot — a fused card has surrendered independent existence entirely, ownership-based zone-return included. Contrast an **attached** card (Z.6), which remains its own object and does return to its owner: the P.8 cascade sends each attached card to *its own* owner's EXILE, not the host's owner's — so a shed sleeve or a stolen payment comes home even while the fused mutation does not. (This is only tractable because the game is digital: the engine tracks each object's owner as a stable field regardless of where it physically sits, so a card owned by one player and fused inside another player's creature is never ambiguous.)
