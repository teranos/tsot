...
- **Z.6** ATTACHED — a card placed under another card.
  - **Z.6a** An attached card occupies its own sleeve alongside the host. It remains movable, strippable, and targetable independent of the host. Examples: hand-pitched payments (P.6), attach-cost payments (P.31).
- **Z.7** SAME-SLEEVE — a card sharing the host's sleeve. A sleeve holds at most 4 cards: a host and up to 3 other cards not already in a sleeve.
  - **Z.7a** When a sleeve holds more than one card, added cards enter front-facing and go to the back position. The outermost card is visible; where it is transparent, the card behind it shows through — V.8's stacking rule applies within a sleeve as well as on a DECK.
- **Z.8** CARDLESS SLEEVE — a sleeve with no card in it (an "empty sleeve"). It has no symbol and cannot be cast.
  ...
  - **Z.8d** An effect — or a HAND payment (P.6) — can attach it to any host (C.14 no longer gates attachment by frame; a cardless sleeve has no frame in any case). While attached it can pay an ATTACHED cost.
  - **Z.8e** A cardless sleeve is transparent when its sleeve is transparent (the default) — with no card inside, nothing blocks V.8's reveal of the card immediately below.
...
- **C.4** Card sleeves are transparent — they exist as protection only and never alter what's visible on either face. The back of a card carries real game information (symbols per C.1, color per C.5, see-through windows for transparent-frame cards per C.13 + V.8), so an opaque physical sleeve would hide it. Opaque cardless sleeves (Z.8, V.8b) are a distinct concept.
...
- **C.10** A spell that is played resolves to GRAVEYARD. Its effects apply after the card has left HAND and arrived in GRAVEYARD.
...
- **C.14** *(Former restriction lifted.)* Previously a `transparent`-frame card could be attached or same-sleeved only to another `transparent`-frame card. **That limitation no longer applies.** Any card — transparent-frame or not — may be attached or same-sleeved to any host, across every attachment mechanism in the game: HAND-source payments (P.6), ATTACHED-source payments (P.31), Mutation casts (P.26, which produce same-sleeve per Z.7), and any redirected attachment (e.g., destroy-and-attach effects). Frame no longer gates attachment; a transparent-frame attachee onto a non-transparent host is legal. (Transparent-frame cards remain valid HAND-source payments for non-BOARD casts — spells, mutations — subject to P.7a identity matching, exactly as before, since those payments resolve to GRAVEYARD rather than attaching.)
...
- **P.11** A cost component written as `N mill` means: place the top N cards of your DECK into your GRAVEYARD. A cardless sleeve on top does not count toward N — it goes uncounted into the GRAVEYARD along the way, and the mill reaches past it to card-bearing sleeves, so the cost stays payable with cardless sleeves on top.
...
- **P.12b** When a cast's cost includes any GRAVEYARD-source component paid in compliance with P.12a, the HAND-source payments for that cast are exempt from P.7a. The color-matching GRAVEYARD pitch supplies the thematic alignment for the entire payment bundle, so any cards in the controller's HAND may pay the HAND components (subject to count and timing; C.14's former frame-based attachment restriction is lifted). P.7a still governs casts whose cost has no GRAVEYARD-source component.
- **P.13** A cost component of `N hand` is valid on a card of any type. How the HAND payments come to rest depends on where the cast card goes: on a BOARD-placed card they attach to it face-down (P.6 / P.17); on a non-permanent spell (which has no BOARD host) they follow the spell-payment convention and are discarded to the GRAVEYARD on resolution. (Amended 2026-07-18: the original P.13 restricted `hand` to BOARD-placed cards; Bitter Dawn — a white instant costing `1 hand + 2 graveyard` — casts with a `hand` component and the engine already routes its HAND payment to the GRAVEYARD, so the card led and the rule was brought in line rather than the card changed.)
...
- **P.26** A mutation card is played by targeting a creature on the BOARD. The mutation does not enter the BOARD itself: it joins the targeted creature in the same sleeve (Z.7). HAND-source payments for the mutation do not attach to the host — they resolve to GRAVEYARD per the spell-payment convention (C.10).
...
- **P.28** A mutation's effects (statics, granted keywords, granted activated abilities) apply to its host creature for as long as the host is on the BOARD. Static effects apply regardless of whether the host is face-up or face-down; being same-sleeve does not suppress them.
- **P.29** When the host creature moves to any zone (GY, EXILE, HAND, DECK, or back to BOARD), its same-sleeve mutations move with it to the same zone, for the remainder of the game (Z.7). Mutations are not subject to P.8's attached-cascade to EXILE — they are same-sleeve (Z.7), not attached (Z.6).
...
- **P.30** A variable-X cost component has a minimum X value of 1. A card may explicitly permit X = 0 on its printed rule.
...
- **P.40** Some creatures have a printed rule that replaces their death. Such a rule triggers before the creature reaches the GRAVEYARD.
- **P.40a** A creature whose death is prevented remains on the BOARD with its accumulated damage cleared.
- **P.40b** A creature whose death is redirected moves to the named zone instead of the GRAVEYARD. On-death triggers do not fire, and P.8's attached-cascade does not run.
- **P.40c** A creature may leave its own sleeve as part of the replacement (Z.8). The emptied sleeve then attaches to the card (Z.6). Ownership persists (T.3). A given sleeve can be shed only once.
...
- **P.42 — `N tap` cost component.** Choose N untapped permanents you control on the BOARD and tap them (turn sideways, as in B.4) to pay the cost. The permanents are **not consumed**: they stay on the BOARD and untap as normal at your next Untap (U.2). (Numbered P.40 on the feature branch; renumbered to P.42 in the merge with master's P.40 death-replacement.)
  - **P.42a — Color anchor.** A cast with a `tap` component must have at least one payment sharing a printed color with the card being cast. The anchor may be supplied by **any** single color-matching payment in the cost — a tapped permanent, a HAND card (P.6), or a GRAVEYARD card (P.12); any one qualifies, regardless of source. Remaining payments may be any color or colorless; a colorless card can never anchor. Same one-anchor spirit as P.12a (`graveyard`) and P.12b (a color anchor from one source covering another).
    - **Tap-only cost** → taps are the only payments, so the anchor must be an on-color tap. The color rule always applies.
    - **Mixed cost** (`tap` + `hand` and/or `gy`) → a color-matching tap, HAND, *or* GRAVEYARD payment qualifies equally. Since P.12a already forces a color-matching graveyard pitch, any `gy + tap` cost is auto-anchored.
  - **P.42b — Color only.** The anchor tests printed *colors*, not symbols — contrast P.7a (colors ∪ symbols). The `transparent` frame is not a color and never satisfies it.
  - **P.42c — Any card type.** A `tap` component may sit on a card of any type. Unlike `N hand` (P.13), which discards its payment to the GRAVEYARD on a spell, a `tap` payment is never attached or consumed — the tapped permanents stay on the BOARD (P.42) — so there is likewise no BOARD-placement restriction.
  - **P.42d — Summoning sickness.** B.3 does not prevent a creature from being tapped to pay `tap` — this uses the permanent as a resource, distinct from A.6's `T:` activation of the creature's own ability (which does gate on B.3). A creature that entered the BOARD this turn may still be tapped to pay `tap`.
...
- **A.7** A creature that attacks (B.1) is considered to have attacked this turn until the start of the next turn.
...
- **A.11** **P/T modifications resolve before other effects.** Whenever a triggered, activated, or static effect reads a creature's X or Y stat, it observes the effective post-modifier value — every stat-modifying static, attached buff, debuff, and modifier-stacking adjustment has already applied at the moment of the read. A same-turn buff is visible to a same-turn trigger; a creature whose toughness was just modified mid-turn reflects the new value to any subsequent read.
- **A.12** **Cost reductions resolve before other effects.** Whenever a triggered, activated, or static effect reads a card's combined cost (or any other cost field), it observes the effective post-reduction value — every cost-reducing static and modifier has already applied at the moment of the read. An effect like "destroy target creature with combined cost ≥ N" reads the *reduced* cost when the target sits under a cost-reduction static, matching A.11's contract for toughness buffs.
- **A.13 — Tap / untap as an effect.** An effect that taps or untaps a card sets or clears that card's tapped state (B.4 / U.2); the controller chooses the target per A.3.
  - **Any card, either side.** Any card on either player's BOARD is a legal target.
  - **No-op, never illegal.** If the card is already in the target state, the effect does nothing, but the target is still legal — tapping a tapped card, or untapping an untapped one, is allowed and simply has no effect.
  - **No color match.** Targets are unconstrained by color; the anchor rule (P.42a) governs *paying*, not what an effect may touch.
  - **Distinctness not required.** An effect that both untaps and taps may pick the same card for both (untap → tap leaves it tapped, a legal no-op).
...
- **T.3** A same-sleeve card (Z.7) has no independent zone existence — it is a child of its host, not a card occupying any zone of its own. It moves and rests with its host wherever the host goes, including into an opponent's zones when cast on an opponent's creature (P.27, P.29), for the remainder of the game. It is not counted, targeted, or moved as an independent object (C.16, V.7b).
- **T.3a** T.2's ownership-based zone-return does not apply to a same-sleeve card — ownership is rendered moot for it until it leaves same-sleeve.
- **T.3b** Attached cards (Z.6) are distinct: they remain their own object and do return to their owner. The P.8 cascade sends each attached card to *its own* owner's EXILE — a shed sleeve or a stolen payment comes home even while a same-sleeve mutation does not.
...
- **V.8b** A sleeve's opacity dominates. An opaque sleeve makes its contents non-transparent regardless of the card's frame. Transparency (V.8) requires both the sleeve and the card (if any) to be transparent.
- **V.9** A card with the glow face attribute has visibility determined by its **effective slot** in the DECK, computed by counting only non-transparent-frame cards above it. Transparent-frame cards in slots above are ignored for this computation. Glow is a cosmetic surface treatment, not a color.
