...
- **P.40a — Color anchor.** At least one permanent in the tapped bundle must share a printed color with the card being cast. One match suffices; the remaining permanents may be any color or colorless. A card with no printed colors cannot anchor the check and so cannot be paid for with `tap` (empty color set can't intersect). Same one-anchor model as P.12a (`graveyard`).
