...
- **C.14** A `transparent` card can be attached only to another `transparent` card. The rule applies to every attachment mechanism in the game: HAND-source payments (P.6), ATTACHED-source payments (P.31), Mutation casts (P.26), and any redirected attachment (e.g., destroy-and-attach effects). Concretely: when the host (the card receiving an attachment) is non-`transparent`, a transparent attachee is refused. Transparent ↔ transparent attachments are legal. Non-transparent attachees can attach to any host. Transparent cards remain valid HAND-source payments for non-BOARD casts (spells, mutations), subject to P.7a identity matching, since those payments resolve to GRAVEYARD rather than attaching.
