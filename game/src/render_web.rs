...
struct RenderWebState {
    ...
    // Mesh pipeline — indexed draw of the continuous woody surface
    // (CONTINUOUS_WOOD.md) plus the instanced canopy elements.
    //
    // Wood: every tree's woody skeleton merged into ONE world-space
    // vertex+index buffer, drawn as a single identity `MeshInstance`.
    // The buffers grow with the world (unlike the old baked-once cone),
    // so both are `Option<>` with capacity tracking.
    //
    // Canopy: unit leaf card baked once at init, instanced per station.
    //
    // Instance packing: [wood_identity(1 if wood non-empty)] + [canopy];
...
pub fn init(canvas_id: &str) -> bool {
    ...
    // Bake the canopy card (constant, no inputs). Wood buffers are
    // created lazily on the first frame — their size grows with the
    // world, unlike the old baked-once trunk cone.
...
/// `MeshInstance`, then the canopy elements instanced per station. One
/// instance buffer packs [wood_identity(1)] + [canopy]; `render_mesh`
/// dispatches each with the right vertex/index + first_instance. Wood
/// vertex + index buffers grow as the world's tree count grows. A no-op
/// success when there's neither wood nor canopy in view.
pub fn frame_mesh(
    wood_verts: &[MeshVertex],
    wood_indices: &[u32],
    canopy: &[MeshInstance],
) -> u32 {
    ...
    STATE.with(|c| {
        ...
        // Grow the mesh instance buffer if needed.
        ...
        // covers every tree with a single trunk colour. Same limitation
        // as native, called out in CONTINUOUS_WOOD.md.
        ...
        // Wood pass — grow vertex + index buffers, upload, draw.
        if wood_count == 1 {
