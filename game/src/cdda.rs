...
pub fn mapgen_to_template(
    ...
    // Centre the grid on the anchor: col → +x, row → +z.
    let height = obj.rows.len() as f32;
    let width = obj.rows.iter().map(|r| r.chars().count()).max().unwrap_or(0) as f32;
    let cx = width / 2.0;
    let cz = height / 2.0;
    ...
    for (r, row) in obj.rows.iter().enumerate() {
        for (c, ch) in row.chars().enumerate() {
            if let Some(kind) = cell_to_prop(ch, &obj.terrain, &obj.furniture) {
...
/// The garage, imported from the embedded CDDA mapgen.
pub fn garage_template() -> Result<Template, CddaError> {
    mapgen_to_template(GARAGE_JSON, "s_garage_1", CDDA_TILE)
}
...
mod tests {
    ...
    #[test]
    fn imports_garage_with_walls_and_furniture() {
        let t = garage_template().expect("garage should import");
        let n = |k: PropKind| t.props.iter().filter(|p| p.kind == k).count();
        assert!(n(PropKind::Wall) > 10, "expected a wall outline, got {}", n(PropKind::Wall));
        assert!(
