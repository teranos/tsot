...
// Version picker — press 9 to open/close. Fetched from
// /versions.json (regenerated by deploy-game.yml on every push).
// Master lives at the bucket root; every other branch sits under
// /<branch>/. The picker synthesises a "master" entry on top of
// whatever S3's CommonPrefixes returned.
function currentBranch(): string {
  const parts = location.pathname.split('/').filter(x => x.length > 0)
  // First non-empty path segment that isn't an html file is the
  // branch prefix; if none, we're on master at root.
  const first = parts[0]
  if (!first || first.endsWith('.html') || first.endsWith('.js')) return 'master'
  return first
}
...
async function loadVersions() {
  ...
  try {
    ...
    const versions = await r.json() as string[]
    const current = currentBranch()
    // Build DOM directly (no HTML injection surface).
    list.textContent = ''
...
function appendVersionLink(list: HTMLElement, href: string, label: string, isCurrent: boolean) {
...
window.addEventListener('keydown', e => {
  if (e.key === '9') {
    const el = document.getElementById('version-picker')
