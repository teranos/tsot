...
# files.txt). This is deliberate: the fetched set — and therefore
# flake.nix's content hash — must NOT depend on files.txt. Adding a
# building is one line in files.txt; it must not change what's fetched
# or force a hash re-pin. build.rs picks the exact files to embed from
# these subtrees.
#
# Output: <crate>/.cdda-src/ (gitignored). build.rs reads from here
# when $CDDA_SRC is unset (the Nix flake sets $CDDA_SRC to the pinned
# input instead, so this script is only for bare-cargo / CI builds).
#
# Keep these patterns identical to sparseCheckout in game/flake.nix,
# so the script and the flake produce the same tree.
set -eu

here=$(cd "$(dirname "$0")/.." && pwd)          # crates/cdda/
rel=$(tr -d ' \t\r\n' < "$here/RELEASE")
...
# Integrity check: verify the fetched HEAD matches COMMIT. The nix
...
if [ "$actual_commit" != "$expected_commit" ]; then
    ...
    echo "[fetch-cdda] If CleverRaven moved the $rel tag, update COMMIT AND flake.nix's hash together." >&2
    exit 1
fi
# Stamp the release so build.rs can detect a stale .cdda-src (bumping
# RELEASE without a re-fetch would otherwise compile the old corpus
# silently).
printf '%s\n' "$rel" > "$dst/.rev"
